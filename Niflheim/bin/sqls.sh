...
sq.sh|sed -n '2,$p' | while read line; do id=$(echo $line|awk '{print $1}');name=$(echo $line|awk '{print $3}');state=$(echo $line|awk '{print $5}');times=$(echo $line|awk '{print $7}');cpus=$(echo $line|awk '{print $10}');dir=$(showjob $id|grep 'WorkDir'|awk -F'=' '{print $2}'|sed 's\/home/energy/changai\~\g'); printf '%8s %-26s %8s %10s %3s %s/\n' $id $name $state $times $cpus $dir; done
